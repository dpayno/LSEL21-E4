...
{
  fsm_led_alarm_t* f = (fsm_led_alarm_t*)this;
  return get_tick_count() >= get_current_timeout(f->u8_index);
}
...
static void turn_led_off (fsm_t* this)
{
  fsm_led_alarm_t* f = (fsm_led_alarm_t*)this;
  led_off(f->u8_index);
  set_new_timeout(f->u8_index, 0);
}

static void init_timer (fsm_t* this)
{
  fsm_led_alarm_t* f = (fsm_led_alarm_t*)this;
  set_new_timeout(f->u8_index, get_tick_count() + get_blink_time(f->u8_index) );
}
...
static void blink_leds_and_new_timeout (fsm_t* this)
{
  ...
  set_new_timeout(f->u8_index, get_current_timeout(f->u8_index) + get_blink_time(f->u8_index) );
}
...
void fsm_led_alarm_init(fsm_led_alarm_t* this, uint8_t pin, uint16_t blink_time)
{
  ...
  set_blink_time (this->u8_index, ms_into_ticks(blink_time));
}
